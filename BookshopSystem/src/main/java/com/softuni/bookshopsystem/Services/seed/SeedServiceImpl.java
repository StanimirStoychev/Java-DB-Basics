...
public class SeedServiceImpl implements SeedService {
    ...
    private final author.AuthorService authorService;
    private final BookService bookService;
    private final CategoryService categoryService;

    public SeedServiceImpl(author.AuthorService authorService, BookService bookService, CategoryService categoryService) {
    ...
    public void seedBooks() throws IOException {
        final List<Book> books = Files.readAllLines(Path.of(RESOURCE_URL + BOOK_FILE_NAME))
                ...
                .map(row -> {
                    ...
                    return Book.builder()
                            .title(title)
                            .editionType(com.softuni.bookshopsystem.Domain.Enums.EditionType.values()[Integer.parseInt(data[0])])
                            .price(new BigDecimal(data[3]))
                            .releaseDate(LocalDate.parse(data[1], DateTimeFormatter.ofPattern("d/M/yyyy")))
                            .ageRestriction(com.softuni.bookshopsystem.Domain.Enums.AgeRestriction.values()[Integer.parseInt(data[4])])
